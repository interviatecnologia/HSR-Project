...
// FUNÃ‡ÃƒO DE EXTERNAL_PAUSE
// Verifica se a funÃ§Ã£o solicitada Ã© "external_pause"
if ($function == 'external_pause') {
    // VALIDAR DADOS
    // Verifica se os dados de entrada sÃ£o vÃ¡lidos
    if ((strlen($value) < 1) || ((strlen($agent_user) < 1) && (strlen($alt_user) < 1)) || (!preg_match("/PAUSE|RESUME/", $value))) {
        ...
        // Verifica se o usuÃ¡rio tem permissÃ£o para executar a funÃ§Ã£o
        if ((!preg_match("/ $function /", $VUapi_allowed_functions)) && (!preg_match("/ALL_FUNCTIONS/", $VUapi_allowed_functions))) {
            // RESPOSTA DE ERRO
            // Retorna uma resposta de erro em formato JSON
            $response = array(
        ...
        // VERIFICAR AGENTE
        // Verifica se o agente estÃ¡ logado
        $stmt = "select count(*) from vicidial_live_agents where user='$agent_user';";
        if ($DB) {echo "$stmt\n";}
        $rslt = mysql_to_mysqli($stmt, $link);
        $row = mysqli_fetch_row($rslt);
        if ($row[0] > 0) {
            // VERIFICAR SE O AGENTE ESTÃ PAUSADO
            // Verifica se o agente estÃ¡ pausado antes de tentar pausar ou retomar
...
// FUNÃ‡ÃƒO DE LOGOUT
// Verifica se a funÃ§Ã£o solicitada Ã© "logout"
if ($function == 'logout') {
    // VALIDAR DADOS
    // Verifica se os dados de entrada sÃ£o vÃ¡lidos
